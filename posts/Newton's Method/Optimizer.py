...
class LogisticRegression(LinearModel):
    ...
    def hessian(self, X, y):
        ...
        RETURNS:
         hessian, torch.tensor: the Hessian matrix of the loss function
        """""

        s = self.score(X)
        D = torch.sigmoid(s) * (1-torch.sigmoid(s))
        Ddiagonal = torch.diag(D)
        return X.T@Ddiagonal@X
...
class NewtonOptimizer(LogisticRegression):
    ...
    def step(self, X, y, alpha):
        ...
        #> first update:
            ...
            self.model.w = weight - (alpha*torch.inverse(hessian))@grad
